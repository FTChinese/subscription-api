...
func TestMembership_IsExpired(t *testing.T) {
	profile := subscription.NewProfile()
...
func TestMembership_SubsKind(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				Status: subscription.SubStatusIncompleteExpired,
			},
			args:    args{p: subscription.yearlyStandard},
			want:    plan.SubsKindCreate,
			wantErr: false,
		},
		{
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				...
				PaymentMethod: enum.PayMethodStripe,
				AutoRenewal:   true,
				Status:        subscription.SubStatusActive,
			},
			wantErr: true,
		},
		{
...
func TestMembership_MergeIAPMembership(t *testing.T) {
	...
	expiredFtc := subscription.NewProfile().SetExpireDate(time.Now().AddDate(0, -1, 0)).Membership()
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				m: subscription.NewProfile().Membership(),
			},
			args: args{
				iapMember: Membership{},
			},
		...
		},
		{
			...
			fields: fields{
				m: subscription.NewProfile().SetPayMethod(enum.PayMethodApple).Membership(),
			},
			args: args{
				iapMember: Membership{},
			},
...
func TestMembership_IsEqual(t *testing.T) {
	profile := subscription.NewProfile().SetPayMethod(enum.PayMethodApple)
