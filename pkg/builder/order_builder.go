...
type OrderBuilder struct {
	...
	membership subs.Membership // User's current membership. Needs querying DB.
	wallet     subs.Wallet     // Only required if kind == SubsKindUpgrade.

	// Optional fields.
	client          client2.Client      // Only requird when actually creating an order.
...
func (b *OrderBuilder) SetClient(c client2.Client) *OrderBuilder {
	b.client = c
	return b
}
...
func (b *OrderBuilder) generateOrderID() (err error) {
	...
	b.orderID, err = subs.GenerateOrderID()
	return
}
...
func (b *OrderBuilder) SetMembership(m subs.Membership) *OrderBuilder {
	b.membership = m
...
func (b *OrderBuilder) SetWallet(w subs.Wallet) *OrderBuilder {
	b.wallet = w
	return b
}

func (b *OrderBuilder) GetWallet() subs.Wallet {
	return b.wallet
}
...
func (b *OrderBuilder) Build() error {
	...
	if b.kind == enum.OrderKindUpgrade && b.upgradeSchemaID.IsZero() {
		b.upgradeSchemaID = null.StringFrom(subs.GenerateUpgradeID())
	}
...
func (b *OrderBuilder) Order() (subs.Order, error) {
...
func (b *OrderBuilder) FreeUpgrade() (subs.ConfirmationResult, error) {
	if err := b.ensureBuilt(); err != nil {
	...
	if !b.IsFreeUpgrade() {
		return subs.ConfirmationResult{}, subscription.ErrBalanceCannotCoverUpgrade
	}
...
func (b *OrderBuilder) PaymentIntent() (subscription.PaymentIntent, error) {
	if err := b.ensureBuilt(); err != nil {
...
func (b *OrderBuilder) UpgradeSchema() (subs.UpgradeSchema, error) {
	if err := b.ensureBuilt(); err != nil {
		return subs.UpgradeSchema{}, err
	}

	if b.kind != enum.OrderKindUpgrade {
		return subs.UpgradeSchema{}, errors.New("not an upgrade subscription")
	}

	return subs.UpgradeSchema{
...
func (b *OrderBuilder) proratedOrdersSchema() []subs.ProratedOrderSchema {
	orders := make([]subs.ProratedOrderSchema, 0)

	for _, v := range b.wallet.Sources {
		s := subs.ProratedOrderSchema{
			ProratedOrder: v,
...
// creating an order.
func (b *OrderBuilder) ClientApp() client2.OrderClient {
	return client2.OrderClient{
		OrderID: b.orderID,
		Client:  b.client,
	}
}
