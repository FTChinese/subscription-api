...
func TestMembership_ValidateMergeIAP(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "FTC is taken by another IAP, IAP empty",
			fields: Membership{
				MemberID: MemberID{
					CompoundID: "",
					FtcID:      null.StringFrom(uuid.New().String()),
		...
		{
			name: "FTC side is valid, IAP empty",
			fields: Membership{
				MemberID: MemberID{
					CompoundID: "",
					FtcID:      null.StringFrom(uuid.New().String()),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := tt.fields
			if got := m.ValidateMergeIAP(tt.args.iapMember); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ValidateMergeIAP() = %v, want %v", got, tt.want)
			}
		})
	}
}
