...
func (env Env) ConfirmOrder(pr ftcpay.PaymentResult, order ftcpay.Order) (ftcpay.ConfirmationResult, *ftcpay.ConfirmError) {
	...
	if err != nil {
		sugar.Error(err)
		return ftcpay.ConfirmationResult{}, pr.ConfirmError(err.Error(), true)
	}
	...
	if err != nil {
		sugar.Error(err)
		_ = tx.Rollback()
		return ftcpay.ConfirmationResult{}, pr.ConfirmError(err.Error(), err != sql.ErrNoRows)
	}
	...
	if order.IsConfirmed() {
		...
		if order.IsExpireDateSynced(member) {
			...
			_ = tx.Rollback()
			return ftcpay.ConfirmationResult{
				Payment:    pr,
				Order:      order,
	...
	confirmed, err := ftcpay.NewConfirmationResult(ftcpay.ConfirmationParams{
		Payment: pr,
		Order:   order,
		Member:  member,
	})
...
func (env Env) SaveConfirmErr(e *ftcpay.ConfirmError) error {
	_, err := env.dbs.Write.NamedExec(
		ftcpay.StmtSaveConfirmResult,
		e)
...
func (env Env) SavePayResult(result ftcpay.PaymentResult) error {
	if result.OrderID == "" {
		return nil
	}

	_, err := env.dbs.Write.NamedExec(ftcpay.StmtSavePayResult, result)
	if err != nil {
		return err
	}
