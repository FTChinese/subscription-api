...
func TestAccountTx_BaseAccountForStripe(t *testing.T) {
...
func TestAccountTx_SavedStripeID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := tx.SavedStripeID(tt.args.a); (err != nil) != tt.wantErr {
				t.Errorf("SavedStripeID() error = %v, wantErr %v", err, tt.wantErr)
				_ = tx.Rollback()
				return
			}
