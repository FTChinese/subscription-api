...
func (router AuthRouter) RequestSMSVerification(w http.ResponseWriter, req *http.Request) {
	...
	mobileFound, err := router.Repo.SearchByMobile(params.Mobile)
	if err != nil {
		sugar.Error(err)
...
func (router AuthRouter) VerifySMSCode(w http.ResponseWriter, req *http.Request) {
	...
	result, err := router.Repo.SearchByEmail(account.MobileEmail(params.Mobile))
	if err != nil {
		sugar.Error(err)
...
func (router AuthRouter) MobileLinkExistingEmail(w http.ResponseWriter, req *http.Request) {
	...
	authResult, err := router.Repo.Authenticate(params.EmailCredentials)
	if err != nil {
		sugar.Error(err)
		_ = render.New(w).DBError(err)
		return
	...
	acnt, err := router.ReaderRepo.AccountByFtcID(authResult.UserID)
	if err != nil {
	...
	// in current db.
	mobileSearch, err := router.Repo.SearchByMobile(params.Mobile)
	if err != nil {
		sugar.Error(err)
		_ = render.New(w).DBError(err)
		return
	...
	err = router.Repo.UpsertMobile(account.MobileUpdater{
		FtcID:  acnt.FtcID,
