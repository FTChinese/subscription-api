...
type PaywallRouter struct {
	...
	StripePrice stripeenv.PriceStore
	Logger      *zap.Logger
	Live        bool
}
...
func (router PaywallRouter) BustCache(w http.ResponseWriter, req *http.Request) {
	...
	_, err = router.StripePrice.ListPrices(router.Live, true)
	if err != nil {
		sugar.Error(err)
		_ = render.New(w).DBError(err)
		return
	...
	for _, id := range stripeIDs {
		_, err := router.StripePrice.LoadPrice(id, false)
		if err != nil {
			sugar.Error(err)
			_ = render.New(w).DBError(err)
			return
	...
	stripePrices, err := router.StripePrice.ListPrices(router.Live, false)
	if err != nil {
		_ = render.New(w).DBError(err)
		return
	}
